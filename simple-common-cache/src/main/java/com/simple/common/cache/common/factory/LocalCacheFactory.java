...
public class LocalCacheFactory<K, V> {
     ...
     */
    private final Executor removalListenerExecutor = ForkJoinPool.commonPool();
     ...
     */
    public Cache<K, V> createCache(String cacheName, Consumer<CacheSpec<K, V>> configurator) {
    ...
    private <C extends Cache<K, V>> C getOrCreate(String cacheName, CacheType cacheType, Consumer<CacheSpec<K, V>> configurator) {
        // 构建复合键（名称+类型）
     ...
     * 2. 按需设置各配置参数
     */
    private void applyConfig(Caffeine<Object, Object> builder, CacheSpec<K, V> spec) {
        // 初始容量（可选）
        ...
        // 自动刷新配置（需要加载器）
        if (spec.refreshAfterWrite > 0 && spec.loader != null) {
    ...
    /**
     * 缓存配置规格类（核心设计），使用 -1 表示未配置，配置详细介绍见 applyConfig
     * 设计理念：
     * 1. 集中管理所有配置参数
     * 2. 提供链式配置方法
     * 3. 封装校验逻辑
    ...
    public static class CacheSpec<K, V> {
         ...
         */
        void validate() {
            boolean hasEvictionPolicy = maximumSize > 0 || expireAfterWrite > 0 || expireAfterAccess > 0 || softValues;
