...
- **强制打包**：任何代码修改后，必须 `mvn clean package -DskipTests` 并确保 `BUILD SUCCESS`。
  - 优先使用 `package` 而非 `compile`，因为 package 会执行完整构建流程（编译、资源处理、依赖检查、JAR 包生成）
  - 能更全面地验证代码正确性，发现 compile 阶段无法发现的问题
  - 如果模块非常大，可先用 `compile` 快速验证语法，但最终以 `package` 成功为准
- **打包失败**：查看错误 → 定位 → 修复 → 重新打包，直至成功。
